...
try {
...

	{
		...
		ok(
			"bash keeps an execute",
			typeof h.tools.get("bash").execute === "function",
		);
		ok(
			"bash keeps its parameters schema",
			h.tools.get("bash").parameters !== undefined,
		);
		ok(
			"bash keeps its prompt snippet",
			typeof h.tools.get("bash").promptSnippet === "string",
		);
		ok(
			"edit defines both render slots",
			typeof h.tools.get("edit").renderCall === "function" &&
				typeof h.tools.get("edit").renderResult === "function",
		);
	}
	...
	{
		const ownPath = fileURLToPath(
			new URL("../../delta/index.ts", import.meta.url),
		);
		...
		ok(
			"a lost tool name warns",
			h
				.messages()
				.some(
					(m) => m.includes("bash") && m.includes("owned by another extension"),
				),
			JSON.stringify(h.messages()),
		);
		ok(
			"it names the thief's path",
			h.messages().some((m) => m.includes(foreignPath)),
			JSON.stringify(h.messages()),
		);
		ok(
			"it does not warn about the name delta still owns",
			!h
				.messages()
				.some((m) => m.includes("edit") && m.includes("owned by another")),
			JSON.stringify(h.messages()),
		);
	}

	{
		...
		await h.fire("session_start");
		ok(
			"no warning when delta wins every name",
			h.messages().filter((m) => m.includes("owned by another")).length === 0,
			JSON.stringify(h.messages()),
		);
	}

	{
		...
		ok(
			...
			!h.messages().some((m) => m.includes("owned by another")),
			JSON.stringify(h.messages()),
		);
	}

	{
		...
		await writeFile(
			...
			JSON.stringify({ enabled: false, command: "delta-does-not-exist" }),
		);
		const h = createFakePi({
			cwd: project,
			hasUI: true,
			getAllTools: () => [
				{
					name: "bash",
					sourceInfo: { path: "/else/conflicting/bash.ts", source: "local" },
				},
			],
		});
		extension(h.pi);
		await h.fire("session_start");
		ok(
			"the collision warns even with enabled:false",
			h.messages().some((m) => m.includes("owned by another")),
			JSON.stringify(h.messages()),
		);
...

	{
		await writeFile(
			...
			JSON.stringify({ shellCommandPrefix: "export DELTA_PREFIX_RAN=yes" }),
		);
		...
		const cwdResult = await bash.execute(
			"call-cwd",
			{ command: "pwd" },
			undefined,
			undefined,
		...
		ok(
			"bash runs in the session's cwd",
			text(cwdResult).includes(project),
			JSON.stringify(text(cwdResult)),
		);

		const prefixResult = await bash.execute(
			"call-prefix",
			{ command: 'echo "$DELTA_PREFIX_RAN"' },
			undefined,
			undefined,
			ctx,
		);
		ok(
			"bash honours shellCommandPrefix",
			text(prefixResult).includes("yes"),
			JSON.stringify(text(prefixResult)),
		);
		...
		const editResult = await h.tools
			...
			.execute(
				"call-edit",
				{ path: "edited.txt", edits: [{ oldText: "before", newText: "after" }] },
				undefined,
				undefined,
				ctx,
			);
		ok(
			"edit resolves relative paths against the session's cwd",
			!editResult.isError,
			JSON.stringify(editResult),
		);
		ok(
			"edit actually edited the session's file",
			(await readFile(join(project, "edited.txt"), "utf8")).trim() === "after",
		);
...

	{
		...
		await h.fire("session_start");
		ok(
			"malformed config warns",
			h.messages().some((m) => m.includes("invalid JSON")),
			JSON.stringify(h.messages()),
		);
		await writeFile(
			join(agentDir, "delta.json"),
			JSON.stringify({ command: "delta-does-not-exist" }),
		);
	}
...

	{
		...
		const pendingCall = edit.renderCall(args, theme, {
			...baseContext,
			lastComponent: undefined,
		});
		const pendingPainted = pendingCall.render(80).join("\n");
		ok(
			"pending call shows exactly one header",
			headerCount(pendingPainted) === 1,
			JSON.stringify(pendingPainted),
		);
		...
		const result = {
			content: [
				{ type: "text", text: "Successfully replaced 1 block(s) in g.txt." },
			],
			details: {
				diff: "-old\n+new",
				patch: "diff --git a/g.txt b/g.txt\n@@ -1 +1 @@\n-old\n+new\n",
			},
		};
		const resultComponent = edit.renderResult(
			result,
			{ expanded: false, isPartial: false },
			theme,
			{ ...baseContext, lastComponent: undefined },
		);
		...
		ok(
			"settled edit shows exactly one header across both slots",
			headerCount(combined) === 1,
			JSON.stringify({ settledCallPainted, settledResultPainted }),
		);
		ok(
			"result slot paints nothing itself",
			settledResultPainted.length === 0,
			JSON.stringify(settledResultPainted),
		);
		ok(
			"the settled diff is visible",
		...
		const secondCall = edit.renderCall(args, theme, {
			...baseContext,
			lastComponent: pendingCall,
		});
		const secondCombined = `${secondCall.render(80).join("\n")}\n${resultComponent.render(80).join("\n")}`;
		ok(
			"re-rendering the call slot still shows exactly one header",
			headerCount(secondCombined) === 1,
			JSON.stringify(secondCombined),
		);
		ok(
			"re-rendering the call slot keeps the settled diff",
			secondCombined.includes("+new"),
			JSON.stringify(secondCombined),
		);
	}
...

	{
		...
		const result = {
			content: [
				{ type: "text", text: "Successfully replaced 1 block(s) in f.txt." },
			],
			details: {
				diff: "-1 a\n+1 b",
		...
		// preceding call.
		const callComponent = edit.renderCall(args, theme, {
			...baseContext,
			lastComponent: undefined,
		});
		const resultComponent = edit.renderResult(
		...
		ok(
			"result slot renders nothing",
			resultComponent.render(80).length === 0,
			JSON.stringify(resultComponent.render(80)),
		);
		...
		ok(
			"edit result shows pi's fallback diff",
			/\+1 /.test(painted),
			JSON.stringify(painted),
		);
		ok(
			"edit result shows the header",
			painted.includes("edit") && painted.includes("f.txt"),
			JSON.stringify(painted),
		);
		await settle(200);
		...
		const secondCall = edit.renderCall(secondArgs, theme, {
			...secondContext,
			lastComponent: undefined,
		});
		edit.renderResult(
			{
				content: [
					{ type: "text", text: "Successfully replaced 1 block(s) in f2.txt." },
				],
				details: {
					diff: "-2 c\n+2 d",
					patch: "diff --git a/f2.txt b/f2.txt\n@@ -1 +1 @@\n-c\n+d\n",
				},
			},
			{ expanded: false, isPartial: false },
			theme,
...

	{
		...
		ok(
			"two distinct contexts were minted",
			first !== second && h.contexts.length === 3,
		);
		ok(
			"the superseded session wrote nothing",
			first.own.notices.length === 0,
...

	{
		...
		const plain = bash.renderResult(
			result,
			{ expanded: false, isPartial: false },
			theme,
			{ ...base, args: { command: "echo hello" } },
		);
		ok(
			"non-diff command still renders",
			plain !== undefined && typeof plain.render === "function",
		);

		const diff = bash.renderResult(
			{
				content: [
					{ type: "text", text: "diff --git a/f b/f\n@@ -1 +1 @@\n-a\n+b" },
				],
				details: undefined,
			},
			{ expanded: false, isPartial: false },
		...
		ok(
			"diff command renders our component",
			typeof diff.update === "function",
			typeof diff.update,
		);
		ok(
			"non-diff command renders pi's, not ours",
			typeof plain.update !== "function",
			typeof plain.update,
		);
...

	{
		...
		ok(
			"an error after a partial diff render does not throw",
			thrown === undefined,
			String(thrown),
		);
		ok(
			"the error render falls back to pi's component",
			errored !== undefined && typeof errored.update !== "function",
		);
		...
		ok(
			"the error component renders",
			renderThrew === undefined,
			String(renderThrew),
		);
	}
...

	{
		...
		ok(
			"ANSI from the command is stripped",
			!frame.includes("\x1b"),
		...
		ok(
			"the diff itself survives",
			frame.includes("+b") && frame.includes("@@ -1 +1 @@"),
			JSON.stringify(frame),
		);
	}
...

	{
		...
		const tooWide = (lines, width) =>
			lines.filter((line) => visibleWidth(line) > width);
		...
		ok(
			"edit header fits the render width",
			tooWide(editCall.render(60), 60).length === 0,
			JSON.stringify(editCall.render(60).map((l) => visibleWidth(l))),
		);
		...
		ok(
			"settled edit frame fits the render width",
			tooWide(editCall.render(60), 60).length === 0,
			JSON.stringify(editCall.render(60).map((l) => visibleWidth(l))),
		);
		...
		const errorContext = {
			...editContext,
			state: {},
			toolCallId: "call-wide-err",
		};
		const editErrorCall = edit.renderCall(args, theme, errorContext);
		edit.renderResult(
			{
				content: [{ type: "text", text: `edit failed: ${long}` }],
				details: undefined,
			},
			{ expanded: false, isPartial: false },
			theme,
...

	{
		...
		await writeFile(
			join(agentDir, "delta.json"),
			JSON.stringify({ command: fakeDelta }),
		);
		...
		const result = {
			...
			details: {
				diff: "-a\n+b",
				patch: "diff --git a/s.txt b/s.txt\n@@ -1 +1 @@\n-a\n+b\n",
			},
		};
		...
		ok(
			"a live session is repainted when delta answers",
			live.invalidations === 1,
			String(live.invalidations),
		);
		ok(
			"delta's output reaches the frame",
			live.painted.includes("DELTA-RENDERED"),
			JSON.stringify(live.painted),
		);

		const torn = await paint({ shutdown: true });
		ok(
			"a run landing after session_shutdown does not repaint",
			torn.invalidations === 0,
			String(torn.invalidations),
		);
