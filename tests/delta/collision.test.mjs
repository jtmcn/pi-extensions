...
const { toolCollisions, sameRegistrationPath } = await loadExt("delta/collision.ts");
...
ok("we own both names -> no collision", toolCollisions([
	{ name: "bash", path: mine, source: "local" },
	{ name: "edit", path: mine, source: "local" },
], mine, ["bash", "edit"], identity).length === 0);

ok("a foreign extension owns bash -> reported", (() => {
	const lost = toolCollisions([
		{ name: "bash", path: foreign, source: "local" },
		{ name: "edit", path: mine, source: "local" },
	], mine, ["bash", "edit"], identity);
	return lost.length === 1 && lost[0].name === "bash" && lost[0].owner === foreign;
})());

ok("a foreign owner is reported by its path", (() => {
...
ok("losing both names reports both", (() => {
	const lost = toolCollisions([
		{ name: "bash", path: foreign, source: "local" },
...
ok("same path through a symlink is the same file", sameRegistrationPath(realFile, linkFile) === true);
ok("a symlink and an unrelated file are different", sameRegistrationPath(linkFile, join(dir, "other.ts")) === false);
...
ok("nonexistent paths compare lexically", sameRegistrationPath("./a.ts", `${process.cwd()}/a.ts`) === true);
ok("distinct nonexistent paths differ", sameRegistrationPath("a.ts", "b.ts") === false);
